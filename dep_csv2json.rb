...
def line_map(lines)
  ...
  #   map[index] = {}
  #   map[index]['entries'] = entries(line)
  #   map[index]['depth'] = depth(line)
  map = []
  lines.each do |line|
    p depth(line)
    map.append depth(line)
  end
  map
end

def subset_lines(lines, index = 0, depth = 0)
  subset = []
  lines[index..].each do |line|
    counter += 1
    index += 1
    p "line = #{line}"
    p "depth of line = #{depth(line)}"
    p "depth = #{depth}"
    p "index = #{index}"
    line = subset_lines(lines, index, depth(line)) if depth(line) > depth + 1
...
if __FILE__ == $PROGRAM_NAME
  ...
  # build_data(csv)
  lines = get_lines(csv)
  # pp subset_lines(lines)
  p line_map(lines).length
  # new_obj = determine_type(lines, 0).new
  # p new_obj.class
  # data = parse_lines(lines, new_obj)
end